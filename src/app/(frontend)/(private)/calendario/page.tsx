...
import { obtenerCitas } from './actions/obtenerCitas'
...
const sampleEvents: CalendarEvent[] = [
  ...
  },
  {
    id: '14',
    title: 'My test',
    start: setMinutes(setHours(subDays(new Date(), 9), 16), 0), // 1:00 PM, 9 days before
    end: setMinutes(setHours(subDays(new Date(), 9), 18), 30), // 3:30 PM, 9 days before
  },
]
...
export default async function Page() {
  ...
  const events: CalendarEvent[] = citas.map(
    (cita): CalendarEvent => ({
      id: cita.id.toString(),
      title: (cita.customer as Customer).firstName + ' ' + (cita.customer as Customer).lastName,
      start: parseISO(cita.fechaHora),
      end: addHours(parseISO(cita.fechaHora), 1),
    }),
  )
